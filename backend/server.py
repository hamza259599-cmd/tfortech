...
async def admin_list_testimonials(request: Request):
    await require_admin(request)
    items = await db.testimonials.find({}, {"_id": 0}).sort("sort_order", 1).to_list(500)
    return items

@api_router.post("/admin/testimonials")
async def create_testimonial(item: TestimonialCreate, request: Request):
    await require_admin(request)
    count = await db.testimonials.count_documents({})
    doc = {
        "id": f"tst_{uuid.uuid4().hex[:12]}",
        **item.model_dump(),
        "sort_order": count,
        "created_at": datetime.now(timezone.utc).isoformat()
    }
...
async def update_testimonial(testimonial_id: str, request: Request):
    await require_admin(request)
    body = await request.json()
    result = await db.testimonials.update_one({"id": testimonial_id}, {"$set": body})
    if result.matched_count == 0:
...
async def get_public_testimonials():
    items = await db.testimonials.find({"is_active": True}, {"_id": 0}).sort("sort_order", 1).to_list(500)
    return items
